...
if (($remfiles.count -ne 0) -or ($newfiles.count -ne 0) -or ($filechange.count -ne 0)) {
    ...
    if ($remfiles.count -ne 0) {
        ...
        foreach ($rfile in $remfiles) {
            ...
            $rprop = [ordered]@{
                ...
                'Change Type' = 'Removed'
                'Last Update' = $(Get-Date -Format yyyyMMddTHHmm)
            }
            #Create custom object and export removed files to logfile
            $remexp = [PSCustomObject] $rprop
            $remexp | Export-csv -Path (join-path -Path $logs -ChildPath $newchange) -Append -NoTypeInformation
        }
    }
    ...
    if ($newfiles.count -ne 0) {
        ...
        foreach ($nfile in $newfiles) {
            ...
            $newexp = [PSCustomObject] $nprop
            $newexp | Export-csv -Path (join-path -Path $logs -ChildPath $newchange) -Append -NoTypeInformation
        }
    }
    ...
    if ($filechange.count -ne 0) {
        ...
        foreach ($fchange in $filechange) {
            ...
            $changeexp = [PSCustomObject] $fprop
            $changeexp | Export-csv -Path (join-path -Path $logs -ChildPath $newchange) -Append -NoTypeInformation
        }
    }
    ...
    Write-Output -InputObject ('Writing to Logfile - {0}\{1} ' -f $logs, $newchange)
}
